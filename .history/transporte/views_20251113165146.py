...
def dashboard(request):
    ...
    # Últimos despachos
    ultimos_despachos = Despacho.objects.all().order_by('-fecha_despacho')[:5]
    
    # Despachos por estado (para gráfico)
    despachos_por_estado = {
        'PENDIENTE': despachos_pendientes,
        'EN_RUTA': despachos_en_ruta,
        'ENTREGADO': despachos_entregados,
        'CANCELADO': Despacho.objects.filter(estado='CANCELADO').count()
    }
